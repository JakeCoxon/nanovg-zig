export const createWebGLContext = (canvas, config) => {
  ...
  const glInitShader = (sourcePtr, sourceLen, type) => {
    const source = config.wasmEnv.readCharStr(sourcePtr, sourceLen);
  ...
  const glShaderSource = (shader, count, string, length) => {
    const strs = new Uint32Array(config.memory.buffer, string, count);
    const lens = new Uint32Array(config.memory.buffer, length, count);
    let source = '';
    for (let i = 0; i < count; i++) {
      source += config.wasmEnv.readCharStr(strs[i], lens[i]);
    }
  ...
  const glGetShaderiv = (shader, pname, params) => {
    const buffer = new Uint32Array(config.memory.buffer, params, 1);
  ...
  const jsBindAttribLocation = (programId, index, namePtr, nameLen) => gl.bindAttribLocation(glPrograms[programId], index, config.wasmEnv.readCharStr(namePtr, nameLen));
  ...
  const glUniform4fv = (locationId, count, value) => {
    let arr = new Float32Array(config.memory.buffer, value, count * 4);
    gl.uniform4fv(glUniformLocations[locationId], arr);
  }
  const glUniformMatrix3fv = (locationId, count, transpose, dataPtr) => {
    const floats = new Float32Array(config.memory.buffer, dataPtr, count * 9);
  ...
  const glUniformMatrix4fv = (locationId, count, transpose, dataPtr) => {
    const floats = new Float32Array(config.memory.buffer, dataPtr, count * 16);
  ...
  const glUniform2fv = (locationId, count, value) => {
    let arr = new Float32Array(config.memory.buffer, value, count * 2);
  ...
  const glGenBuffers = (num, dataPtr) => {
    const buffers = new Uint32Array(config.memory.buffer, dataPtr, num);
    for (let n = 0; n < num; n++) {
  ...
  const glBufferData = (type, count, dataPtr, drawType) => {
    const floats = new Uint8Array(config.memory.buffer, dataPtr, count);
  ...
  const glLoadTexture = (urlPtr, urlLen) => {
    const url = config.wasmEnv.readCharStr(urlPtr, urlLen);
  ...
  const glGenTextures = (num, dataPtr) => {
    const textures = new Uint32Array(config.memory.buffer, dataPtr, num);
    for (let n = 0; n < num; n++) {
  ...
  function jsLoadTextureIMG(dataPtr, dataLen, mimePtr, mimeLen, widthPtr, heightPtr) {
    const data = new Uint8Array(config.memory.buffer, dataPtr, dataLen);
    const textureId = glCreateTexture();
    const img = new Image();
    // images.push(img); // track loading progress
    const mime = config.wasmEnv.readCharStr(mimePtr, mimeLen);
    img.src = URL.createObjectURL(new Blob([data], { type: mime }));
    img.onload = () => {
      if (widthPtr) new Uint16Array(config.memory.buffer, widthPtr, 1)[0] = img.width;
      if (heightPtr) new Uint16Array(config.memory.buffer, heightPtr, 1)[0] = img.height;
      createGLTexture(img, glTextures[textureId]);
    };
    return textureId;
  }
  function jsLoadTextureIMG2(handleId, image) {
    const textureId = glTextures[handleId];
    if (!textureId) throw new Error('Texture not found');
    gl.bindTexture(gl.TEXTURE_2D, textureId);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);
  }

  const getTextureFromHandle = (handleId) => glTextures[handleId];
  ...
  const glGenVertexArrays = (num, dataPtr) => {
    const vaos = new Uint32Array(config.memory.buffer, dataPtr, num);
    for (let n = 0; n < num; n++) {
  ...
  const glReadPixels = (x, y, w, h, format, type, pixels) => {
    const data = new Uint8Array(config.memory.buffer, pixels);
  ...
  const glGenFramebuffers = (n, framebuffers) => {
    const buffers = new Uint32Array(config.memory.buffer, framebuffers, n);
    for (let i = 0; i < n; i++) {
  ...
  const glGenRenderbuffers = (n, renderbuffers) => {
    const buffers = new Uint32Array(config.memory.buffer, renderbuffers, n);
    for (let i = 0; i < n; i++) {
  ...
  const glGetIntegerv = (pname, param) => {
    ...
    const buffers = new Uint32Array(config.memory.buffer, param, result.length);
